...
struct FolderDetailToolbarView: View {
    ...
    var body: some View {
        ...
        let stackSpacing: CGFloat = isPadLandscape ? 18 : 25

        Group {
            if isPadLandscape {
                VStack(spacing: stackSpacing) {
                    toolbarButtons(isOnboarding: isOnboarding)
                    Spacer()
                }
            } else {
        ...
        .padding(isPadLandscape ? .vertical : .horizontal, 12)
        .padding(isPadLandscape ? .horizontal : .vertical, 12)
    ...
    @ViewBuilder
    private func toolbarButtons(isOnboarding: Bool) -> some View {
        if showSearchButton && !uiState.isSelectionMode {
